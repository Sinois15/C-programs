...
void calcTotal(int x){
    ...
    for (int i= 0; i<receipts[x].iCount; i++){
        ...
        receipts[x].total = receipts[x].total + (receipts[x].items[i].price * receipts[x].items[i].quantity);
    }
}
...
void createReceipt(){
    ...
    while (choice == 'y'){ //loop iterates while user wants to enter item details
        ...
        do{
            ...
            if(receipts[count].items[receipts[count].iCount].price < 0){
        ...
        do{
            ...
            if(receipts[count].items[receipts[count].iCount].quantity < 0){
